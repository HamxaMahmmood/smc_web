...
export async function POST(req: NextRequest) {
  try {
    ...
    // age arrives as a string like "3" or "14" from ageValue
    // guard against undefined/null/empty string but allow "0" (newborns)
    ...
    // ── Retry loop for MR number race condition ───────────────
    ...
    for (let attempt = 0; attempt < MAX_RETRIES; attempt++) {
      try {
        ...
        // Only retry on duplicate MR number
        ...
        // Any other error — throw immediately, don't retry
        throw err;
      }
    }
